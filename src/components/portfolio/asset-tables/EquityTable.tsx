...
import { BaseAssetTable, createPnLCell, createPercentCell } from './BaseAssetTable';
...
export function EquityTable({ holdings }: EquityTableProps) {
      ...
      cell: ({ row }) => (
        <div className="flex flex-col min-w-[150px]">
          ...
          <div className="flex items-center gap-2 text-xs text-muted-foreground">
            ...
            <span>•</span>
            <span>{row.original.exchange}</span>
          </div>
        </div>
      ),
    },
  ...
  return (
    <BaseAssetTable
      ...
      emptyMessage="No equity holdings found"
    />
  );
}
